...
class Cliente extends Page {
    ...
    public static function editClientes($request){

        $id = $_SESSION['pages']['usuario']['id'];

        $cliente = M_Cliente::buscar('codcli = '.$id,null,null,'*')->fetchObject(M_Cliente::class);

        $content = View::render('pages/cliente_form',[
            'codigo' => $cliente->codcli,
            'cliente' => $cliente->cliente,
    ...
    public static function atualizaClientes($request){
        ...
        $obCliente->instagram = $postVars['instagram'];
        $obCliente->senha = $postVars['senha'];

        $obCliente->atualizar();

        return self::editClientes($request);
    }
